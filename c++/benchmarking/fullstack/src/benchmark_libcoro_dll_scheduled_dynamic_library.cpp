...
namespace comprehensive::v1
{
        ...
        size_t blob_size)
    {
        ...
        auto root_service = rpc::root_service::create(
            "benchmark_libcoro_dll_scheduled_dynamic_library", rpc::DEFAULT_PREFIX, scheduler);
        ...
        auto child_transport = std::make_shared<rpc::libcoro_dll_scheduled_dynamic_library::child_transport>(
            "benchmark_libcoro_dll_scheduled_dynamic_library", root_service, CANOPY_BENCHMARK_LIBCORO_DLL_SCHEDULED_DLL_PATH);
...

        {
            auto connect_result = CO_AWAIT root_service->connect_to_zone<i_data_processor, i_data_processor>(
                "benchmark_libcoro_dll_scheduled_dynamic_library", child_transport, not_used);

...
namespace comprehensive::v1
{
        ...
        size_t blob_size)
    {
        ...
        auto root_service = rpc::root_service::create("benchmark_shared_scheduler_dll", rpc::DEFAULT_PREFIX, scheduler);
        root_service->set_default_encoding(enc);

        auto child_transport = std::make_shared<rpc::shared_scheduler_dll::child_transport>(
            "benchmark_shared_scheduler_dll", root_service, CANOPY_BENCHMARK_SHARED_SCHEDULER_DLL_PATH);
...

        {
            auto connect_result = CO_AWAIT root_service->connect_to_zone<i_data_processor, i_data_processor>(
                "benchmark_shared_scheduler_dll", child_transport, not_used);

...
#  include "../../streaming/websocket_client_stream.h"
...
namespace comprehensive::v1
{
    namespace
    {
            ...
            uint16_t port)
        {
            ...
            coro::sync_wait(
                coro::when_all(
                    [&]() -> coro::task<void>
                    {
                        ...
                        server_stream
                            = std::make_shared<streaming::tcp::stream>(std::move(*accepted), pair.server_scheduler);
                    }(),
                    [&]() -> coro::task<void>
                    {
                        ...
                        client_stream = std::make_shared<streaming::tcp::stream>(std::move(client), pair.client_scheduler);
                    }()));
            ...
            pair.server = std::make_shared<streaming::tcp::stream>(streaming::tcp::socket(server_fd));
            pair.client = std::make_shared<streaming::tcp::stream>(streaming::tcp::socket(client_fd));
            return true;
#endif
        }
...
#  endif
        ...
        bool wrap_websocket(benchmark_stream_pair& pair)
        {
            ...
            pair.client = std::make_shared<stream_bench::websocket_client_stream>(pair.client);
            return true;
        }
#endif

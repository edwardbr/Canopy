...
namespace stream_bench
{
    ...
    {
        const char* valid_streams =
#ifdef CANOPY_BUILD_WEBSOCKET
            "spsc, tcp, io_uring, tls+spsc, ws+spsc, tls+ws+spsc";
#else
            "spsc, tcp, io_uring, tls+spsc";
#endif
        fmt::print(
            "Usage: {} [options]\n"
            "\n"
    ...
    static bench_config parse_args(
        int argc,
        char** argv)
    {
        bench_config cfg;
...
namespace stream_bench
{
    ...
    void print_stress_row(
        const char* type,
        const stress_stats& send_s,
        const stress_stats& recv_s)
    {
        if (!send_s.valid)
        {
    ...
    void run_spsc_based_stress_bench(
        const char* name,
        std::shared_ptr<streaming::stream> side_a,
        std::shared_ptr<streaming::stream> side_b,
        const bench_config& cfg,
    ...
    if (cfg.run_unidirectional || cfg.run_send_reply)
    {
        ...
        collect(
            ...
            [&](size_t blob_size, bench_stats& uni, bench_stats& reply)
            {
                ...
                auto ws_client = std::make_shared<bench_helpers::ws_client_stream>(pipe->side_b(sched_b));
        ...
        if (tls_cert.valid)
        {
            collect(
                ...
                [&](size_t blob_size, bench_stats& uni, bench_stats& reply)
                {
                    ...
                    auto ws_client = std::make_shared<bench_helpers::ws_client_stream>(tls_b);

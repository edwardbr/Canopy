...
namespace stream_bench
{
    ...
    namespace
    {
        ...
        bool make_tls_websocket_pair(
            ...
            std::unique_ptr<spsc_pipe>& pipe,
            tls_fixture_cert_pair& cert)
        {
            sched_a = make_scheduler();
            sched_b = make_scheduler();
            pipe = std::make_unique<spsc_pipe>();
        ...
        void run_standard_tls_websocket_spsc(
            ...
            bench_stats& send_reply)
        {
            ...
            std::shared_ptr<streaming::stream> server;
            std::shared_ptr<streaming::stream> client;
            if (make_tls_websocket_pair(server, client, sched_a, sched_b, pipe, cert))
        ...
        void run_stress_tls_websocket_spsc(
            const bench_config& cfg,
            watchdog& wd,
            size_t blob_size,
            stress_stats& send,
    ...
    {
#ifdef CANOPY_BUILD_WEBSOCKET
        if (!should_run_stream(cfg, "tls+ws+spsc"))
            return;
        ...
        if (cfg.run_unidirectional || cfg.run_send_reply)
        {
            for (const auto blob_size : get_blob_sizes(cfg))
            {
                standard_jobs.push_back(
                    standard_benchmark_job{"tls+ws+spsc",
                        blob_size,
                        [&cfg, &wd, blob_size](bench_stats& unidirectional, bench_stats& send_reply)
                        { run_standard_tls_websocket_spsc(cfg, wd, blob_size, unidirectional, send_reply); }});
            }
        }
        ...
        if (cfg.run_stress)
        {
            for (const auto blob_size : get_stress_blob_sizes(cfg))
            {
                stress_jobs.push_back(
                    stress_benchmark_job{"tls+ws+spsc",
                        blob_size,
                        [&cfg, &wd, blob_size](stress_stats& send, stress_stats& recv)
                        { run_stress_tls_websocket_spsc(cfg, wd, blob_size, send, recv); }});
            }
        }
#else
        (void)cfg;

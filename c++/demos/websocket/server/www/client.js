...
// Quality slider 0..100 -> VP8 encoder (bitrate_kbps, cpu_used).
...
// (stop+start keeps the websocket; the server re-inits its codec when the
...
function connect() {
    ...
    transport = new CanopyUntrustedWebTransport({
        ...
        onOpen: function(t) {
            ...
            // Sync the server to the current control state on connect.
            sendVideoEffects();
            sendVideoParams();
        },
...
//   RPC into the server, no awaiting.
//
...
// will work unchanged once the server starts genuinely transforming frames.
//
...
// The server processes frames serially and the receive loop blocks on the
// outbound push, so sustained throughput is well under 15 fps. Producing at
// ~10 fps keeps the inbound rate at/below what the server drains so no
// multi-second kernel-socket backlog forms. A worker that processes only the
// latest frame (roadmap) would lift this.
// The server now self-regulates (drops stale frames), so the browser can
// capture at a higher rate — more frames give the server fresher choices
...
// Modest bitrate at 320x240: smaller frames decode faster in the server and
...
async function startVideo() {
    ...
    try {
        ...
        // frame at the display refresh rate, so irregular network/server
...
function handleRemoteFrame(seq, pts_us, flags, payload) {
    ...
    try {
        ...
        // End-to-end latency: this frame's seq round-tripped from the server.

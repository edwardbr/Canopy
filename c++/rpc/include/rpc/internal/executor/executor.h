...
#  ifdef FOR_SGX
        const auto thread_count = std::max<uint32_t>(options.thread_count, 1U);
        scheduler_options.thread_count = thread_count;
        scheduler_options.pool.thread_count = thread_count;
#  else
...
namespace rpc
{
    inline executor_ptr make_executor(executor_options = {})
    {

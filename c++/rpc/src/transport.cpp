...
namespace rpc
{
        ...
        destination_zone inbound_source)
    {
        ...
        pass_thoughs_[lookup_val] = std::move(pt);
    ...
    void transport::decrement_outbound_proxy_count(destination_zone dest)
    {
        ...
        {
            std::unique_lock lock(destinations_mutex_);
            should_shutdown = inner_decrement_outbound_proxy_count(dest, transport_to_remove);
        }

        if (transport_to_remove.is_set())
        {
            if (auto svc = service_.lock())
            {
                svc->remove_transport_if_matches(transport_to_remove, this);
                RPC_DEBUG(
                    "decrement_outbound_proxy_count: All counts reached 0 for zone={}, requested transport removal",
                    transport_to_remove.get_subnet());
            }
        }
    ...
    void transport::decrement_inbound_stub_count(caller_zone dest)
    {
        ...
        {
            std::unique_lock lock(destinations_mutex_);
            should_shutdown = inner_decrement_inbound_stub_count(dest, transport_to_remove);
        }
        ...
        if (transport_to_remove.is_set())
        {
            if (auto svc = service_.lock())
            {
                svc->remove_transport_if_matches(transport_to_remove, this);
                RPC_DEBUG(
                    "decrement_inbound_stub_count: All counts reached 0 for zone={}, requested transport removal",
                    transport_to_remove.get_subnet());
            }
        }
        if (should_shutdown)
    ...
    bool transport::inner_decrement_outbound_proxy_count(
        ...
        destination_zone& transport_to_remove)
    {
        ...
        RPC_DEBUG(
            "decrement_outbound_proxy_count {} -> {} total={} proxy_count_before={} stub_count={} outbound_pt={} "
            "inbound_pt={}",
            zone_id_.get_subnet(),
            dest.get_subnet(),
            get_destination_count(),
            counts.proxy_count.load(),
            counts.stub_count.load(),
            counts.outbound_passthrough_count.load(),
            counts.inbound_passthrough_count.load());
    ...
    bool transport::inner_decrement_inbound_stub_count(
        caller_zone dest,
        destination_zone& transport_to_remove)
    {
        zone dest_zone = dest;
        ...
        RPC_DEBUG(
            "decrement_inbound_stub_count {} -> {} total={} stub_count_before={} proxy_count={} outbound_pt={} "
            "inbound_pt={}",
            zone_id_.get_subnet(),
            dest.get_subnet(),
            get_destination_count(),
        ...
        uint64_t count)
    {
        ...
        {
            std::unique_lock lock(destinations_mutex_);
            trigger_shutdown = inner_decrement_inbound_stub_count_by(dest, count, transport_to_remove);
        }
        if (transport_to_remove.is_set())
        {
            if (auto svc = service_.lock())
            {
                svc->remove_transport_if_matches(transport_to_remove, this);
                RPC_DEBUG(
                    "decrement_inbound_stub_count_by: All counts reached 0 for zone={}, requested transport removal",
                    transport_to_remove.get_subnet());
            }
        }
        if (trigger_shutdown)
    ...
    {
        bool should_shutdown = false;
        bool outbound_transport_reached_zero = false;
        bool inbound_transport_reached_zero = false;
        {
            std::unique_lock lock(destinations_mutex_);
            auto lookup_val = make_pass_through_key(outbound_dest, inbound_source);
            ...
            else
            {
                ...
                else
                {
                        ...
                        && outbound_counts.inbound_passthrough_count == 0)
                    {
                        ...
                        outbound_transport_reached_zero = true;
                    }
                }
            }
            ...
            else
            {
                ...
                else
                {
                        ...
                        && inbound_counts.outbound_passthrough_count == 0 && inbound_counts.inbound_passthrough_count == 0)
                    {
                        ...
                        inbound_transport_reached_zero = true;
                    }
                }
            }
        ...
        if (outbound_transport_reached_zero || inbound_transport_reached_zero)
        {
            if (auto svc = service_.lock())
            {
                if (outbound_transport_reached_zero)
                {
                    // Atomic check-and-remove: avoids the TOCTOU that would arise
                    // from separate get_transport / remove_transport lock acquisitions.
                    svc->remove_transport_if_matches(outbound_dest, this);
                    RPC_DEBUG("remove_passthrough: Removed transport for outbound zone={}", outbound_dest.get_subnet());
                }

                if (inbound_transport_reached_zero)
                {
                    svc->remove_transport_if_matches(inbound_source, this);
                    RPC_DEBUG("remove_passthrough: Removed transport for inbound zone={}", inbound_source.get_subnet());
                }
            }
        }
    }
    ...
    CORO_TASK(void) transport::notify_all_destinations_of_disconnect()
    {
...

        {
            ...
            {
                const auto& counts = zone_item.second;
                if (counts.proxy_count.load(std::memory_order_acquire) > 0
                    || counts.stub_count.load(std::memory_order_acquire) > 0)
                {
    ...
    transport::inbound_object_released(object_released_params params)
    {
        ...
        if (params.caller_zone_id == get_zone_id())
        {
            ...
            if (service)
                CO_AWAIT service->object_released(params);
            CO_RETURN;
        }
        ...
        if (auto telemetry_service = rpc::telemetry::get_telemetry_service(); telemetry_service)
        {
            ...
            else
            {
                ...
                telemetry_service->message(
                    {rpc::telemetry::i_telemetry_service::level_enum::err, "failed to call outbound_send"});
            }
        }
#endif
        CO_RETURN result;
    ...
    CORO_TASK(standard_result) transport::try_cast(try_cast_params params)
    {
        ...
        if (auto telemetry_service = rpc::telemetry::get_telemetry_service(); telemetry_service)
        {
            ...
            else
            {
                // RPC_ASSERT(false);
                telemetry_service->message(
                    {rpc::telemetry::i_telemetry_service::level_enum::err, "failed to call outbound_try_cast"});
            }
        }
#endif
        CO_RETURN result;
    ...
    CORO_TASK(standard_result) transport::add_ref(add_ref_params params)
    {
        ...
        if (auto telemetry_service = rpc::telemetry::get_telemetry_service(); telemetry_service)
        {
            ...
            else
            {
                // RPC_ASSERT(false);
                telemetry_service->message(
                    {rpc::telemetry::i_telemetry_service::level_enum::err, "failed to call outbound_add_ref"});
            }
        }
#endif
        CO_RETURN result;
    ...
    CORO_TASK(standard_result) transport::release(release_params params)
    {
        ...
        if (auto telemetry_service = rpc::telemetry::get_telemetry_service(); telemetry_service)
        {
            ...
            else
            {
                // RPC_ASSERT(false);
                telemetry_service->message(
                    {rpc::telemetry::i_telemetry_service::level_enum::err, "failed to call outbound_release"});
            }
        }
#endif
        CO_RETURN result;
    ...
    CORO_TASK(void) transport::inbound_post_report(rpc::telemetry_event event)
    {
        auto svc = service_.lock();
        if (svc)
            CO_AWAIT svc->post_report(std::move(event));
    }

...
namespace canopy::security::attestation::detail
{
    namespace
    {
        ...
        public:
            ...
            {
                append_string(value.security_domain_id);
        ...
        void append_canonical_domain_pair(
        ...
        auto make_development_shared_secret_context(const security_context& context) -> std::optional<std::vector<uint8_t>>
        {
            ...
            // Multiple zones can share an domain identity, so the pair is
            // canonicalised by domain id before key material is derived.
            append_canonical_domain_pair(encoder, context.local_identity, context.peer_identity);

...
namespace canopy::security::attestation
{
    namespace
    {
        ...
        {
            return value.security_domain_id + "/" + value.zone_id;
        }
        ...
        auto identity_has_domain(const identity& value) -> bool
        {
            return !value.security_domain_id.empty();
        }
            ...
            const security_context& context) -> bool
        {
            if (!context.established || scope.session_id != context.session_id)
                return false;
            if (!identity_has_domain(scope.caller_identity) || !identity_has_domain(scope.destination_identity))
                return false;

            const auto& local = context.local_identity.security_domain_id;
            const auto& peer = context.peer_identity.security_domain_id;
            const auto caller_matches
                = scope.caller_identity.security_domain_id == local || scope.caller_identity.security_domain_id == peer;
            const auto destination_matches = scope.destination_identity.security_domain_id == local
                                             || scope.destination_identity.security_domain_id == peer;

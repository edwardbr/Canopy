...
#if defined(FOR_SGX) && defined(MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG)
extern "C" auto mbedtls_psa_external_get_random(
    mbedtls_psa_external_random_context_t* context,
    uint8_t* output,
    size_t output_size,
    size_t* output_length) -> psa_status_t
{
    (void)context;
    if (!output_length || (!output && output_size != 0))
    ...
    *output_length = 0;
    ...
    if (sgx_read_rand(output, output_size) != SGX_SUCCESS)
...
namespace streaming::mbedtls
{
    namespace
    {
            ...
            size_t output_size) -> int
        {
            ...
            return sgx_read_rand(output, output_size) == SGX_SUCCESS ? 0 : MBEDTLS_ERR_ENTROPY_SOURCE_FAILED;
#else
...
#  endif
            ...
            std::string& out) -> bool
        {
            ...
            (void)path;
            (void)out;
            RPC_ERROR("Mbed TLS file path credentials are unavailable inside the enclave; use PEM credentials");
            return false;
#else

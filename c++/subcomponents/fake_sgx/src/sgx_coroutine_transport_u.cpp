...
extern "C"
{
    sgx_status_t sgx_coroutine_init_enclave(
        ...
        uint64_t request_type_id)
    {
        ...
        auto* raw_symbol = call.symbol("sgx_coroutine_init_enclave");
        if (!raw_symbol)
    ...
    sgx_status_t sgx_coroutine_enter_thread(
        sgx_enclave_id_t enclave_id,
        int* retval,
        std::size_t req_sz,
        const char* req)
    {
        ...
        *retval = 0;
        ...
        auto* raw_symbol = call.symbol("sgx_coroutine_enter_thread");
        if (!raw_symbol)
            return SGX_ERROR_UNEXPECTED;

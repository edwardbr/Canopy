...
namespace rpc::io_uring::detail
{
    ...
    // and reads directly from the completion queue (CQ). Ring memory is a
    // kernel-owned ABI surface rather than ordinary object storage.
    //
    // Submission:
    //
    ...
    // descriptor from host Linux. Keep the fixed kernel ABI layout here instead
    // of exposing liburing types through the common controller surface.
    struct sqe_64
    {
        uint8_t opcode{0};
        uint8_t flags{0};

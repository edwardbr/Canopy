...
namespace
{
    ...
    {
        auto acceptor = CO_AWAIT rpc::ipc_spsc_transport::make_peer_acceptor<yyy::i_host, yyy::i_example>(
            "ipc_spsc_peer_acceptor",
            service,
            rpc::ipc_spsc_transport::shared_memory_file_options{
        ...
        std::atomic_bool& done)
    {
        auto transport = rpc::ipc_spsc_transport::make_peer_client(
            ...
            service,
            rpc::ipc_spsc_transport::shared_memory_file_options{.path = std::move(shared_memory_file)});
        if (!transport)
        {
            exit_code = 4;
            done.store(true, std::memory_order_release);

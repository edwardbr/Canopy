...
// Test using a generated host registry call for schema validation
TEST(
    JSONSchemaValidationTest,
    InprocSetupStandardTests)
{
    // Serialize a call to call_host_look_up_app_not_return with parameter false
    std::vector<char> buffer;
    auto err = yyy::i_example::proxy_serialiser<rpc::serialiser::yas, rpc::encoding>::call_host_look_up_app_not_return(
        "schema-test", false, buffer, rpc::encoding::yas_json);
    ...
    // Find the call_host_look_up_app_not_return function in the real function info
    auto it = std::find_if(
        fi.begin(), fi.end(), [](const auto& func) { return func.name == "call_host_look_up_app_not_return"; });

    ASSERT_NE(it, fi.end()) << "Should find call_host_look_up_app_not_return function";
    ...
    const auto* name = find_member(payload_json, "name");
    ASSERT_NE(name, nullptr) << "Payload should contain name parameter";
    EXPECT_EQ(name->get_type(), json::v1::object::type::string_type) << "name should be a string";
    EXPECT_EQ(name->get<std::string>(), "schema-test") << "name should match the serializer argument";
    ...
    InputOutputSchemaSeparation)
{
    ...
    auto it
        = std::find_if(fi.begin(), fi.end(), [](const auto& func) { return func.name == "call_host_create_local_zone"; });

    ASSERT_NE(it, fi.end()) << "Should find call_host_create_local_zone function";

...
// Test DLL init function for the libcoro_dll_scheduled_dynamic_library transport tests.
...
extern "C" CANOPY_LIBCORO_DLL_SCHEDULED_DLL_EXPORT int canopy_libcoro_dll_scheduled_dll_test_increment_static_probe()
{
...
{
    // Concrete init coroutine called by canopy_libcoro_dll_scheduled_dll_start on the DLL-owned scheduler.
    coro::task<rpc::connect_result> canopy_libcoro_dll_scheduled_dll_init(
        void* transport_ctx,
        const rpc::connection_settings* settings)
    {
        canopy_libcoro_dll_scheduled_dll_test_increment_static_probe();

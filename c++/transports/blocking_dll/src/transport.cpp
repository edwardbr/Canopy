...
// This file is part of transport_blocking_dll and is NOT linked into
// DLL payloads.
//
// dlclose timing:
...
#  if defined(_WIN32)
        ...
        if (!lib_handle_)
        {
            RPC_ERROR("[blocking_dll] LoadLibrary failed for {}", library_path_);
...
#  else
        ...
        if (!lib_handle_)
        {
            RPC_ERROR("[blocking_dll] dlopen failed for {}: {}", library_path_, dlerror());
            return rpc::error::TRANSPORT_ERROR();
        }
#  endif
        ...
        {
            RPC_ERROR("[blocking_dll] one or more canopy_dll_* entry points missing in {}", library_path_);
            unload_library();
            return rpc::error::TRANSPORT_ERROR();
        }
...
#  endif
        ...
        connection_settings input_descr)
    {
        ...
        {
            RPC_ERROR("[blocking_dll] get_new_zone_id failed: {}", zone_result.error_code);
        ...
        {
            RPC_ERROR("[blocking_dll] canopy_dll_init failed: {}", init_err);
            unload_library();

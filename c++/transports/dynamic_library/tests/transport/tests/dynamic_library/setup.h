...
public:
    ~dll_transport_setup() override = default;

    CORO_TASK(bool) CoroSetUp()
    {
        this->start_telemetry_test();

        this->root_service_ = CO_AWAIT rpc::root_service::create("host", rpc::DEFAULT_PREFIX);
        ...
        auto child_transport = std::make_shared<rpc::dynamic_library::child_transport>(
            "main child", this->root_service_, CANOPY_TEST_DLL_PATH);
    ...
    CORO_TASK(rpc::shared_ptr<yyy::i_example>) create_new_zone()
    {
        ...
        auto child_transport = std::make_shared<rpc::dynamic_library::child_transport>(
            "new child", this->root_service_, CANOPY_TEST_DLL_PATH);

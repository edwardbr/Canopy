...
#  endif
        ...
        auto start_fn = reinterpret_cast<dll_start_fn>(load_symbol(result->lib_handle_, "canopy_ipc_spsc_dll_start"));
        if (!start_fn)
        {
            RPC_ERROR("canopy_ipc_spsc_dll_start not found in {}", library_path);
            result->stop();
            return nullptr;
        }

...
namespace rpc::ipc_spsc
{
    struct transport::state
    {
        ...
        rpc::ipc_spsc::queue_pair* queues = nullptr;
        pid_t child_pid = -1;
    ...
    namespace
    {
        [[nodiscard]] std::string make_temp_file_template()
        {
            ...
            auto template_path = temp_dir / ("canopy_ipc_spsc_" + unique_prefix + "_XXXXXX");
        ...
        [[nodiscard]] std::shared_ptr<rpc::ipc_spsc::queue_pair> map_shared_queue_pair(
            const shared_memory_file_options& options)
        {
            ...
            else
            {
                ...
                if (::fstat(fd, &file_status) != 0
                    || file_status.st_size < static_cast<decltype(file_status.st_size)>(sizeof(rpc::ipc_spsc::queue_pair)))
                {
                    ::close(fd);
                    return {};
                }
            ...
            auto* queues = static_cast<rpc::ipc_spsc::queue_pair*>(
                ::mmap(nullptr, sizeof(rpc::ipc_spsc::queue_pair), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
            ::close(fd);
            ...
            return std::shared_ptr<rpc::ipc_spsc::queue_pair>(
                ...
                {
                    if (mapped_queues)
                        ::munmap(mapped_queues, sizeof(rpc::ipc_spsc::queue_pair));
        ...
        const options&)
    {
        ...
        RPC_ASSERT(::ftruncate(fd, sizeof(rpc::ipc_spsc::queue_pair)) == 0);
...
#  endif
    ...
    void transport::reap_child(const std::shared_ptr<state>& state)
    {
        ...
        if (state->queues)
        {
            ::munmap(state->queues, sizeof(rpc::ipc_spsc::queue_pair));

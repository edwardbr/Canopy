...
{
    using base_type = ipc_spsc_setup_base<UseHostInChild, RunStandardTests, CreateNewZoneThenCreateSubordinatedZone>;

    typename base_type::isolated_child_process first_child_;
    typename base_type::isolated_child_process second_child_;
    std::shared_ptr<rpc::ipc_spsc::transport> second_client_transport_;

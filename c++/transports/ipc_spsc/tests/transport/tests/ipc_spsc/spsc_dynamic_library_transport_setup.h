...
#  include <transports/ipc_spsc/loaded_library.h>
#  include <transports/ipc_spsc/queue_transport.h>
...
{
    rpc::ipc_spsc::queue_pair queues_{};
    std::shared_ptr<rpc::ipc_spsc::loaded_library> loaded_;
    ...
    {
        auto client_transport = rpc::ipc_spsc::make_client("spsc child", this->root_service_, &queues_);
...
public:
    void set_up()
    {
        ...
        loaded_ = rpc::ipc_spsc::loaded_library::load(
            CANOPY_TEST_IPC_SPSC_DLL_PATH, "ipc_spsc", dll_zone_, host_zone_, &queues_.dll_to_host, &queues_.host_to_dll);

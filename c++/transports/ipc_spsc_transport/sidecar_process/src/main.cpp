...
int main(
    ...
    char** argv)
{
    auto bootstrap = rpc::ipc_spsc_transport::sidecar_bootstrap::from_command_line(argc, argv);
    if (!bootstrap)
        return 2;
    ...
    auto loaded = rpc::ipc_spsc_transport::loaded_library::load(
        bootstrap->dll_path(),
    ...
    rpc::ipc_spsc_transport::sidecar_bootstrap::unmap_queue_pair(queues);
    return 0;
}

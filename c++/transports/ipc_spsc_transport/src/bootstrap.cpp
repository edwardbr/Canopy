...
namespace rpc::ipc_spsc_transport
{
    namespace
    {
        constexpr auto SIDECAR_DLL_PATH_ARG = "dll-path";
        constexpr auto SIDECAR_MAPPED_FILE_ARG = "mapped-file";
        constexpr auto SIDECAR_DLL_SUBNET_ARG = "dll-subnet";
        constexpr auto SIDECAR_SCHEDULER_THREAD_COUNT_ARG = "scheduler-thread-count";
    ...
    rpc::ipc_spsc_transport::queue_pair* queue_pair_bootstrap::map_queue_pair() const
    {
    ...
    void queue_pair_bootstrap::unmap_queue_pair(rpc::ipc_spsc_transport::queue_pair* queues)
    {
        if (!queues)
            return;

        ::munmap(queues, sizeof(rpc::ipc_spsc_transport::queue_pair));
    }

    sidecar_bootstrap::sidecar_bootstrap(
        std::string dll_path,
        std::string mapped_file,
        rpc::zone dll_zone,
        size_t scheduler_thread_count)
    ...
    const char* sidecar_bootstrap::dll_path_arg_name()
    {
        return SIDECAR_DLL_PATH_ARG;
    }

    const char* sidecar_bootstrap::mapped_file_arg_name()
    {
        return SIDECAR_MAPPED_FILE_ARG;
    }

    const char* sidecar_bootstrap::dll_subnet_arg_name()
    {
        return SIDECAR_DLL_SUBNET_ARG;
    }

    const char* sidecar_bootstrap::scheduler_thread_count_arg_name()
    {
        return SIDECAR_SCHEDULER_THREAD_COUNT_ARG;
    }

    std::shared_ptr<sidecar_bootstrap> sidecar_bootstrap::from_command_line(
        int argc,
        char** argv)
    {
        args::ArgumentParser parser("Canopy IPC SPSC sidecar process");
        ...
        return std::make_shared<sidecar_bootstrap>(
            args::get(dll_path_arg), args::get(mapped_file_arg), dll_zone, args::get(scheduler_thread_count_arg));
    }

    const std::string& sidecar_bootstrap::dll_path() const
    {
        return dll_path_;
    }

    rpc::zone sidecar_bootstrap::dll_zone() const
    {
        return child_zone();
    }

    std::vector<std::string> sidecar_bootstrap::make_command_line() const
    {
        return {

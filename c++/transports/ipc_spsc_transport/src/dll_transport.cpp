...
#  include <thread>
#  include <transports/ipc_spsc_transport/dll_transport.h>
...
extern "C" CANOPY_IPC_SPSC_DLL_EXPORT void canopy_ipc_spsc_dll_start(
    rpc::ipc_spsc_transport::dll_start_params* params,
    rpc::ipc_spsc_transport::dll_start_result* result)
{
    using namespace rpc::ipc_spsc_transport;
    ...
    ctx->worker = std::thread(
        [name = std::string(params->name ? params->name : "ipc_spsc_transport"),

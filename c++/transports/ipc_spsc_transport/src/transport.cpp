...
namespace rpc::ipc_spsc_transport
{
    struct transport::state
    {
        ...
        rpc::ipc_spsc_transport::queue_pair* queues = nullptr;
        pid_t child_pid = -1;
    ...
    namespace
    {
        [[nodiscard]] std::string make_temp_file_template()
        {
            ...
            auto template_path = temp_dir / ("canopy_ipc_spsc_transport_" + unique_prefix + "_XXXXXX");
        ...
        [[nodiscard]] std::shared_ptr<rpc::ipc_spsc_transport::queue_pair> map_shared_queue_pair(
            const shared_memory_file_options& options)
        {
            ...
            if (options.create)
            {
                if (::ftruncate(fd, sizeof(rpc::ipc_spsc_transport::queue_pair)) != 0)
                {
                    ::close(fd);
            ...
            else
            {
                ...
                if (::fstat(fd, &file_status) != 0
                    || file_status.st_size
                           < static_cast<decltype(file_status.st_size)>(sizeof(rpc::ipc_spsc_transport::queue_pair)))
                {
                    ::close(fd);
                    return {};
                }
            ...
            auto* queues = static_cast<rpc::ipc_spsc_transport::queue_pair*>(
                ::mmap(nullptr, sizeof(rpc::ipc_spsc_transport::queue_pair), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0));
            ::close(fd);
            ...
            if (options.create)
                new (queues) rpc::ipc_spsc_transport::queue_pair{};

            return std::shared_ptr<rpc::ipc_spsc_transport::queue_pair>(
                queues,
                [path = options.path, unlink_on_destroy = options.unlink_on_destroy](
                    rpc::ipc_spsc_transport::queue_pair* mapped_queues)
                {
                    if (mapped_queues)
                        ::munmap(mapped_queues, sizeof(rpc::ipc_spsc_transport::queue_pair));
                    if (unlink_on_destroy && !path.empty())
                        ::unlink(path.c_str());
                });
        }
        ...
        const options&)
    {
        ...
        RPC_ASSERT(::ftruncate(fd, sizeof(rpc::ipc_spsc_transport::queue_pair)) == 0);
...
#  endif
    ...
    void transport::reap_child(const std::shared_ptr<state>& state)
    {
        ...
        if (state->queues)
        {
            ::munmap(state->queues, sizeof(rpc::ipc_spsc_transport::queue_pair));
...
#  endif
        ...
        std::shared_ptr<rpc::coro::scheduler> scheduler)
    {
        ...
        auto queues = map_shared_queue_pair(options);
        if (!queues)
            return {};
        ...
        if (role == peer_role::connector)
        {
            ...
            receive_queue = std::shared_ptr<::streaming::spsc_queue::queue_type>(queues, &mapped->dll_to_host);
        }
        else
        {
            send_queue = std::shared_ptr<::streaming::spsc_queue::queue_type>(queues, &mapped->dll_to_host);
            receive_queue = std::shared_ptr<::streaming::spsc_queue::queue_type>(queues, &mapped->host_to_dll);
        }

        return std::make_shared<::streaming::spsc_queue::stream>(
            std::move(send_queue), std::move(receive_queue), std::move(scheduler));
    }
    ...
    std::shared_ptr<rpc::stream_transport::transport> make_peer_client(
        ...
        shared_memory_file_options options)
    {
        auto stream = make_shared_memory_stream(
            std::move(options), peer_role::connector, service ? service->get_scheduler() : nullptr);
        if (!stream)
            return {};

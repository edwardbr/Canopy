...
// Shared coroutine ABI types for the libcoro_dll_scheduled_dynamic_library transport.
// Included by both the host (transport_libcoro_dll_scheduled_dynamic_library) and the DLL
// (transport_libcoro_dll_scheduled_dynamic_library_dll).
//
// COROUTINE-BUILD ONLY.  The distinct canopy_libcoro_dll_scheduled_dll_* symbol names
...
// ---------------------------------------------------------------------------
#  if defined(_WIN32)
#    if defined(CANOPY_LIBCORO_DLL_SCHEDULED_DLL_BUILDING)
#      define CANOPY_LIBCORO_DLL_SCHEDULED_DLL_EXPORT __declspec(dllexport)
#    else
#      define CANOPY_LIBCORO_DLL_SCHEDULED_DLL_EXPORT __declspec(dllimport)
#    endif
#  else
#    define CANOPY_LIBCORO_DLL_SCHEDULED_DLL_EXPORT __attribute__((visibility("default")))
#  endif

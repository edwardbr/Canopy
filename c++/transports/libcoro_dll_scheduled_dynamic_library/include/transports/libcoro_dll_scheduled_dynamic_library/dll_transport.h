...
// DLL-side libcoro_dll_scheduled_dynamic_library transport.  Coroutine-build only.
//
// This header is included by shared objects that participate in the
// Canopy libcoro_dll_scheduled_dynamic_library transport.  The shared object supplies
// canopy_libcoro_dll_scheduled_dll_init; the transport library supplies the exported
// canopy_libcoro_dll_scheduled_dll_start entry point.
...
namespace rpc::libcoro_dll_scheduled_dynamic_library
{
    ...
    // transport library. canopy_libcoro_dll_scheduled_dll_start calls this once on the
    // DLL-owned scheduler.

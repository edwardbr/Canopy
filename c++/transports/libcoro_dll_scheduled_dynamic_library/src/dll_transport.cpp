...
//
// Compiled into transport_libcoro_dll_scheduled_dynamic_library_dll, which the DLL links.
// The DLL author only needs to provide canopy_libcoro_dll_scheduled_dll_init.
...
namespace rpc::libcoro_dll_scheduled_dynamic_library
{
        ...
        const rpc::connection_settings* settings)
    {
        ...
        CO_AWAIT scheduler->schedule();
        CO_RETURN CO_AWAIT canopy_libcoro_dll_scheduled_dll_init(runtime, settings);
    }
...
extern "C" CANOPY_LIBCORO_DLL_SCHEDULED_DLL_EXPORT void canopy_libcoro_dll_scheduled_dll_start(
    rpc::libcoro_dll_scheduled_dynamic_library::dll_start_params* params)
{
    using namespace rpc::libcoro_dll_scheduled_dynamic_library;
    ...
    {
        RPC_ERROR("[libcoro_dll_scheduled_dynamic_library] DLL start threw during initialisation");
        start_result.connect_result = rpc::connect_result{rpc::error::TRANSPORT_ERROR(), {}};
    }

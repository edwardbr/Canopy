...
#  ifndef CANOPY_TEST_LIBCORO_DLL_SCHEDULED_DLL_PATH
#    error "CANOPY_TEST_LIBCORO_DLL_SCHEDULED_DLL_PATH must be defined (set by CMake)"
#  endif

template<bool UseHostInChild, bool RunStandardTests, bool CreateNewZoneThenCreateSubordinatedZone>
class libcoro_dll_scheduled_transport_setup
...
public:
    ~libcoro_dll_scheduled_transport_setup() override = default;

    CORO_TASK(bool) CoroSetUp()
    {
        this->start_telemetry_test();

        this->root_service_ = CO_AWAIT rpc::root_service::create("host", rpc::DEFAULT_PREFIX, this->io_scheduler_);
        ...
        auto child_transport = std::make_shared<rpc::libcoro_dll_scheduled_dynamic_library::child_transport>(
            "main child", this->root_service_, CANOPY_TEST_LIBCORO_DLL_SCHEDULED_DLL_PATH);
    ...
    CORO_TASK(rpc::shared_ptr<yyy::i_example>) create_new_zone()
    {
        ...
        auto child_transport = std::make_shared<rpc::libcoro_dll_scheduled_dynamic_library::child_transport>(
            "new child", this->root_service_, CANOPY_TEST_LIBCORO_DLL_SCHEDULED_DLL_PATH);

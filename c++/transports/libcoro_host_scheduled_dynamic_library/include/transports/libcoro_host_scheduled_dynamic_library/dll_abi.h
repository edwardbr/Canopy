...
// Shared coroutine ABI types for the libcoro_host_scheduled_dynamic_library transport.
// Included by both the host (transport_libcoro_host_scheduled_dynamic_library) and the DLL
// (transport_libcoro_host_scheduled_dynamic_library_dll).
//
// COROUTINE-BUILD ONLY.  The distinct canopy_libcoro_host_scheduled_dll_* symbol names
// prevent a non-coroutine DLL from being accidentally loaded.
//
// Design
// ------
// There is a single extern "C" entry point resolved via dlsym:
//   canopy_libcoro_host_scheduled_dll_create(dll_create_params*, dll_create_result*)
//
...
// ---------------------------------------------------------------------------
#  if defined(_WIN32)
#    if defined(CANOPY_LIBCORO_HOST_SCHEDULED_DLL_BUILDING)
#      define CANOPY_LIBCORO_HOST_SCHEDULED_DLL_EXPORT __declspec(dllexport)
#    else
#      define CANOPY_LIBCORO_HOST_SCHEDULED_DLL_EXPORT __declspec(dllimport)
#    endif
#  else
#    define CANOPY_LIBCORO_HOST_SCHEDULED_DLL_EXPORT __attribute__((visibility("default")))
#  endif
...
namespace rpc::libcoro_host_scheduled_dynamic_library
{
    ...
    // inner_connect after canopy_libcoro_host_scheduled_dll_create.  Adopts transport_ctx,
    ...
    // -----------------------------------------------------------------------
    // Parameters passed to canopy_libcoro_host_scheduled_dll_create
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Result filled in by canopy_libcoro_host_scheduled_dll_create
    // -----------------------------------------------------------------------

...
// Host-side libcoro_host_scheduled_dynamic_library transport.  Coroutine-build only.
//
...
// (canopy_libcoro_host_scheduled_dll_create) via dlsym, calls it synchronously to obtain the
...
namespace rpc::libcoro_host_scheduled_dynamic_library
{
    class child_transport : public rpc::transport
    {
        ...
        // These are passed as host_coro_*_fn callbacks to canopy_libcoro_host_scheduled_dll_create.
        ...
        // Load the shared object and resolve canopy_libcoro_host_scheduled_dll_create
        int load_library();

...
// Host-side (child_transport) implementation for the libcoro_host_scheduled_dynamic_library
...
#  if defined(_WIN32)
        ...
        if (!lib_handle_)
        {
            RPC_ERROR("[libcoro_host_scheduled_dynamic_library] LoadLibrary failed for {}", library_path_);
            return rpc::error::TRANSPORT_ERROR();
        }
#  else
        // Do not use RTLD_NODELETE: this transport is meant to prove that a
        // host-scheduled DLL can unload cleanly and reload with fresh state.
        lib_handle_ = dlopen(library_path_.c_str(), RTLD_NOW | RTLD_LOCAL);
        if (!lib_handle_)
        {
            RPC_ERROR("[libcoro_host_scheduled_dynamic_library] dlopen failed for {}: {}", library_path_, dlerror());
            return rpc::error::TRANSPORT_ERROR();
        }
#  endif

        if (!resolve_symbol("canopy_libcoro_host_scheduled_dll_create"))
        {
            RPC_ERROR(
                "[libcoro_host_scheduled_dynamic_library] canopy_libcoro_host_scheduled_dll_create not found in {}",
                library_path_);
            unload_library();
            return rpc::error::TRANSPORT_ERROR();
        }
...
#  endif
    ...
    void child_transport::release_parent()
    {
        auto scheduler = scheduler_;
        auto self = std::static_pointer_cast<child_transport>(shared_from_this());
        std::thread(
            [self = std::move(self), scheduler]() mutable
            {
                while (!scheduler.expired())
    ...
    // inner_connect
    //
    // Step 1 (sync): call canopy_libcoro_host_scheduled_dll_create to construct the DLL-side
        ...
        connection_settings input_descr)
    {
        ...
        {
            RPC_ERROR("[libcoro_host_scheduled_dynamic_library] get_new_zone_id failed: {}", zone_result.error_code);
        ...
        // --- Step 1 (cont.): call canopy_libcoro_host_scheduled_dll_create synchronously ---
        auto create_fn = reinterpret_cast<dll_create_fn>(resolve_symbol("canopy_libcoro_host_scheduled_dll_create"));
        ...
        {
            RPC_ERROR("[libcoro_host_scheduled_dynamic_library] canopy_libcoro_host_scheduled_dll_create returned null context");
            unload_library();
        ...
        {
            RPC_ERROR("[libcoro_host_scheduled_dynamic_library] init_fn failed: {}", init_result.error_code);
            unload_library();

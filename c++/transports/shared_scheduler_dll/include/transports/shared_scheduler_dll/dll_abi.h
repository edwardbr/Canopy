...
// Shared coroutine ABI types for the shared_scheduler_dll transport.
// Included by both the host (transport_shared_scheduler_dll) and the DLL
// (transport_shared_scheduler_dll_runtime).
//
// COROUTINE-BUILD ONLY.  The distinct canopy_shared_scheduler_dll_* symbol names
// prevent a non-coroutine DLL from being accidentally loaded.
//
// Design
// ------
// There is a single extern "C" entry point resolved via dlsym:
//   canopy_shared_scheduler_dll_create(dll_create_params*, dll_create_result*)
//
...
// ---------------------------------------------------------------------------
#  if defined(_WIN32)
#    if defined(CANOPY_SHARED_SCHEDULER_DLL_BUILDING)
#      define CANOPY_SHARED_SCHEDULER_DLL_EXPORT __declspec(dllexport)
#    else
#      define CANOPY_SHARED_SCHEDULER_DLL_EXPORT __declspec(dllimport)
#    endif
#  else
#    define CANOPY_SHARED_SCHEDULER_DLL_EXPORT __attribute__((visibility("default")))
#  endif
...
namespace rpc::shared_scheduler_dll
{
    ...
    // inner_connect after canopy_shared_scheduler_dll_create.  Adopts transport_ctx,
    ...
    // -----------------------------------------------------------------------
    // Parameters passed to canopy_shared_scheduler_dll_create
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Result filled in by canopy_shared_scheduler_dll_create
    // -----------------------------------------------------------------------

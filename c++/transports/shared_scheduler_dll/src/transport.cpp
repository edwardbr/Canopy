...
// Host-side (child_transport) implementation for the shared_scheduler_dll
...
#  if defined(_WIN32)
        ...
        if (!lib_handle_)
        {
            RPC_ERROR("[shared_scheduler_dll] LoadLibrary failed for {}", library_path_);
...
#  else
        ...
        if (!lib_handle_)
        {
            RPC_ERROR("[shared_scheduler_dll] dlopen failed for {}: {}", library_path_, dlerror());
            return rpc::error::TRANSPORT_ERROR();
        }
#  endif

        if (!resolve_symbol("canopy_shared_scheduler_dll_create"))
        {
            RPC_ERROR("[shared_scheduler_dll] canopy_shared_scheduler_dll_create not found in {}", library_path_);
            unload_library();
            return rpc::error::TRANSPORT_ERROR();
        }
...
#  endif
    ...
    // inner_connect
    //
    // Step 1 (sync): call canopy_shared_scheduler_dll_create to construct the DLL-side
        ...
        connection_settings input_descr)
    {
        ...
        {
            RPC_ERROR("[shared_scheduler_dll] get_new_zone_id failed: {}", zone_result.error_code);
        ...
        // --- Step 1 (cont.): call canopy_shared_scheduler_dll_create synchronously ---
        ...
        {
            RPC_ERROR("[shared_scheduler_dll] canopy_shared_scheduler_dll_create returned null context");
            unload_library();
        ...
        {
            RPC_ERROR("[shared_scheduler_dll] init_fn failed: {}", init_result.error_code);
            unload_library();

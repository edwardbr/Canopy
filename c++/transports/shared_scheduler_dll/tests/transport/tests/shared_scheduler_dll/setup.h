...
#  ifndef CANOPY_TEST_SHARED_SCHEDULER_DLL_PATH
#    error "CANOPY_TEST_SHARED_SCHEDULER_DLL_PATH must be defined (set by CMake)"
#  endif

template<bool UseHostInChild, bool RunStandardTests, bool CreateNewZoneThenCreateSubordinatedZone>
class shared_scheduler_dll_setup
    : public transport_setup_base<UseHostInChild, RunStandardTests, CreateNewZoneThenCreateSubordinatedZone>
{
    ...
    std::vector<std::weak_ptr<rpc::shared_scheduler_dll::child_transport>> child_transports_;

public:
    ~shared_scheduler_dll_setup() override = default;
    ...
    CORO_TASK(bool) CoroSetUp()
    {
        ...
        auto child_transport = std::make_shared<rpc::shared_scheduler_dll::child_transport>(
            "main child", this->root_service_, CANOPY_TEST_SHARED_SCHEDULER_DLL_PATH);
    ...
    CORO_TASK(rpc::shared_ptr<yyy::i_example>) create_new_zone()
    {
        ...
        auto child_transport = std::make_shared<rpc::shared_scheduler_dll::child_transport>(
            "new child", this->root_service_, CANOPY_TEST_SHARED_SCHEDULER_DLL_PATH);
        child_transports_.push_back(child_transport);

...
// Host-side (child_transport) implementation for the unshared_scheduler_dll
...
#  endif
        ...
        connection_settings input_descr)
    {
        ...
        {
            RPC_ERROR("[unshared_scheduler_dll] get_new_zone_id failed: {}", zone_result.error_code);
        ...
        loaded->entry_thread = std::thread(
            [this, loaded_ptr, adjacent_zone_id]()
            {
                ...
                {
                    RPC_ERROR(
                        "[unshared_scheduler_dll] dlopen failed for {}: {}", loaded_ptr->library_path, load_error_text());
                    publish_ready(*loaded_ptr, failure);
                    return;
                }

                auto start_fn = reinterpret_cast<dll_start_fn>(
                    resolve_native_symbol(loaded_ptr->lib_handle, "canopy_unshared_scheduler_dll_start"));
                if (!start_fn)
                {
                    RPC_ERROR(
                        "[unshared_scheduler_dll] canopy_unshared_scheduler_dll_start not found in "
                        "{}: {}",
        ...
        {
            RPC_ERROR("[unshared_scheduler_dll] DLL start failed: {}", start_result.connect_result.error_code);
            unload_library();
        ...
        {
            RPC_ERROR("[unshared_scheduler_dll] canopy_unshared_scheduler_dll_start returned incomplete ABI result");
            unload_library();

 ...
 *
 *   Usage:
 *     const transport = new CanopyUntrustedWebTransport({
 *         url,                   // ws:// or wss://
...
(function (root, factory) {
    if (typeof module === 'object' && module.exports) {
        ...
        root.CanopyUntrustedWebTransport = factory(root.WebSocket);
    }
    ...
    function CanopyUntrustedWebTransport(opts) {
        this._url = opts.url;
    ...
    // ---------------------------------------------------------------------------
    CanopyUntrustedWebTransport.prototype.connect = function () {
        var self = this;
        return new Promise(function (resolve, reject) {
            ...
            ws.onclose = function (event) {
                ...
                if (!handshakeDone) {
                    reject(new Error('WebSocket closed before handshake complete.  This is a demo so only one client at a time, llms are greedy things!'));
                }
    ...
    // ---------------------------------------------------------------------------
    CanopyUntrustedWebTransport.prototype._onMessage = function (event) {
        var self = this;
        if (typeof event.data === 'string') {
            self._onText(event.data);
            return;
    ...
    // ---------------------------------------------------------------------------
    CanopyUntrustedWebTransport.prototype.sendText = function (text) {
    ...
    // ---------------------------------------------------------------------------
    CanopyUntrustedWebTransport.prototype.call = function (interfaceId, methodId, requestBytes) {
        var self = this;
        if (!self._connected) {
    ...
    // ---------------------------------------------------------------------------
    CanopyUntrustedWebTransport.prototype.callPost = function (interfaceId, methodId, requestBytes) {
        var self = this;
        if (!self._connected) {
            throw new Error('[Canopy] Not connected');
        }
    ...
    // ---------------------------------------------------------------------------
    CanopyUntrustedWebTransport.prototype.registerStub = function (stub) {
    ...
    // ---------------------------------------------------------------------------
    CanopyUntrustedWebTransport.prototype.disconnect = function () {
        if (this._ws) {
            this._connected = false;
            this._ws.close();
            this._ws = null;
    ...
    // isConnected()
    // ---------------------------------------------------------------------------
    CanopyUntrustedWebTransport.prototype.isConnected = function () {
        return this._connected;
    };

    CanopyUntrustedWebTransport.ENCODING_PROTOCOL_BUFFERS = ENCODING_PROTOCOL_BUFFERS;
    CanopyUntrustedWebTransport.ENCODING_NANOPB = ENCODING_NANOPB;

    return CanopyUntrustedWebTransport;
});

...
if(NOT DEPENDENCIES_LOADED)
  ...
  set_property(CACHE CANOPY_ATTESTATION_BACKEND PROPERTY STRINGS "FAKE" "NULL" "SGX_SIM" "SGX_EPID" "DCAP")
  if(NOT CANOPY_ATTESTATION_BACKEND MATCHES "^(FAKE|NULL|SGX_SIM|SGX_EPID|DCAP)$")
    message(
      FATAL_ERROR
        "Invalid CANOPY_ATTESTATION_BACKEND '${CANOPY_ATTESTATION_BACKEND}', expected FAKE, NULL, SGX_SIM, SGX_EPID, or DCAP")
  endif()

  option(
    CANOPY_PRODUCTION_RELEASE
    "Enable production release guardrails. Production releases reject fake SGX, SGX simulation, fake attestation, and SGX-sim attestation."
    OFF)

  if(CANOPY_PRODUCTION_RELEASE)
    if(CANOPY_ATTESTATION_BACKEND MATCHES "^(FAKE|SGX_SIM)$")
      message(
        FATAL_ERROR
          "CANOPY_PRODUCTION_RELEASE=ON rejects CANOPY_ATTESTATION_BACKEND=${CANOPY_ATTESTATION_BACKEND}. "
          "Use NULL for explicitly unattested production routes or a hardware backend such as SGX_EPID. "
          "Release_sim/fake-SGX build tests must set CANOPY_PRODUCTION_RELEASE=OFF explicitly.")
    endif()

    if(CANOPY_BUILD_ENCLAVE AND CANOPY_SGX_BACKEND STREQUAL "Fake")
      message(
        FATAL_ERROR
          "CANOPY_PRODUCTION_RELEASE=ON rejects CANOPY_SGX_BACKEND=Fake. "
          "Fake SGX is a build/test backend only; set CANOPY_PRODUCTION_RELEASE=OFF for fake-SGX build tests.")
    endif()

    if(CANOPY_BUILD_ENCLAVE AND NOT SGX_HW)
      message(
        FATAL_ERROR
          "CANOPY_PRODUCTION_RELEASE=ON rejects SGX_HW=OFF. "
          "SGX simulation is a build/test backend only; set CANOPY_PRODUCTION_RELEASE=OFF for Release_SGX_Sim tests.")
    endif()

    if(CANOPY_BUILD_ENCLAVE AND NOT "${SGX_MODE}" STREQUAL "release")
      message(
        FATAL_ERROR
          "CANOPY_PRODUCTION_RELEASE=ON requires SGX_MODE=release for enclave builds.")
    endif()
  endif()
  ...
  option(
    CANOPY_ENABLE_DEVELOPMENT_ATTESTATION_BACKENDS
    "Build fake and SGX-simulation attestation backend implementations. This must be OFF for production releases."
  ...
  if(CANOPY_PRODUCTION_RELEASE AND CANOPY_ENABLE_DEVELOPMENT_ATTESTATION_BACKENDS)
    message(
      FATAL_ERROR
        "CANOPY_PRODUCTION_RELEASE=ON requires CANOPY_ENABLE_DEVELOPMENT_ATTESTATION_BACKENDS=OFF. "
        "Set CANOPY_PRODUCTION_RELEASE=OFF for release-like fake/simulation build tests.")
  endif()

  if(NOT CANOPY_ENABLE_DEVELOPMENT_ATTESTATION_BACKENDS AND CANOPY_ATTESTATION_BACKEND MATCHES "^(FAKE|SGX_SIM)$")
    message(
      FATAL_ERROR
  ...
  if(NOT CANOPY_DEFAULT_ENCODING MATCHES "^(NANOPB|PROTOCOL_BUFFERS|CANONICAL_CRYPTO|YAS_BINARY|YAS_JSON|YAS_COMPRESSED_BINARY)$")
  ...
  elseif(CANOPY_DEFAULT_ENCODING STREQUAL "CANONICAL_CRYPTO")
    if(CANOPY_BUILD_CANONICAL_CRYPTO)
      ...
      message(WARNING "CANOPY_DEFAULT_ENCODING=CANONICAL_CRYPTO but CANOPY_BUILD_CANONICAL_CRYPTO=OFF; defaulting to YAS_BINARY")
      set(CANOPY_DEFAULT_ENCODING "YAS_BINARY")
      set(CANOPY_DEFAULT_ENCODING_VALUE "rpc::encoding::yas_binary")
    endif()
  ...
  elseif(CANOPY_ATTESTATION_BACKEND STREQUAL "SGX_SIM")
    set(CANOPY_ATTESTATION_BACKEND_FLAG CANOPY_ATTESTATION_BACKEND_SGX_SIM)
  elseif(CANOPY_ATTESTATION_BACKEND STREQUAL "SGX_EPID")
    set(CANOPY_ATTESTATION_BACKEND_FLAG CANOPY_ATTESTATION_BACKEND_SGX_EPID)
  elseif(CANOPY_ATTESTATION_BACKEND STREQUAL "DCAP")
    set(CANOPY_ATTESTATION_BACKEND_FLAG CANOPY_ATTESTATION_BACKEND_DCAP)
  else()

...
# This script patches a build-local copy of Intel's prepare_sgxssl.sh. It must
# never be run against the source submodule directly.
#
# The copied script needs three Canopy-specific changes:
#   1. Build against the SGX SDK selected by the active CMake preset instead of
#      assuming it is running inside a full linux-sgx source build.
#   2. Force OpenSSL's no-threads option so enclave OpenSSL does not pull in
#      pthread symbols or require sgx_pthread.edl imports.
#   3. In SGX simulation only, skip SGXSSL's CPU-vendor guard. Hardware SGX is
#      Intel-only, but simulation is useful on non-Intel developer hosts; this
...
# the enclave surface smaller by forcing the copied OpenSSL build script to add OpenSSL's no-threads option. The
# OpenSSL build script may be downloaded by prepare_sgxssl.sh, so this check/sed pair is injected immediately before
...
string(REPLACE
       ...
       _canopy_sgxssl_script "${_canopy_sgxssl_script}")

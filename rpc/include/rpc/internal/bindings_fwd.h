...
namespace rpc
{
    ...
    template<class Ptr> struct interface_bind_result
    {
        ...
        interface_bind_result(int error_code, Ptr iface)
    ...
    struct remote_object_bind_result
    {
        ...
        remote_object_bind_result(int error_code, std::shared_ptr<rpc::object_stub> stub, rpc::remote_object descriptor)
            : error_code(error_code)
            , stub(std::move(stub))
    ...
    create_interface_stub(service* serv, rpc::shared_ptr<T> iface, caller_zone caller_zone_id);

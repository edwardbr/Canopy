...
fn main()
{
    ...
    let repo_root = manifest_dir.parent().and_then(Path::parent).and_then(Path::parent).expect("repo root");

    if let Ok(lib_dir) = env::var("CANOPY_PROTOBUF_NATIVE_LIB_DIR")
    {
        let lib_name = env::var("CANOPY_PROTOBUF_NATIVE_LIB_NAME").unwrap_or_else(|_| "upbd".to_string());
...
fn build_libupb(repo_root: &Path)
{
    ...
    sources.push(proto_root.join("upb/reflection/cmake/google/protobuf/descriptor.upb_minitable.c"));
...
{
    let relative = source.strip_prefix(proto_root).expect("source under protobuf root");
    let name = relative.to_string_lossy().replace('/', "_").replace('.', "_");
    format!("{name}.o")
}

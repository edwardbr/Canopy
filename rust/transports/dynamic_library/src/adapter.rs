...
use crate::ffi::{
    CanopyAddRefParams, CanopyDllInitParams, CanopyGetNewZoneIdParams, CanopyObjectReleasedParams, CanopyPostParams,
    CanopyReleaseParams, CanopySendParams, CanopyTransportDownParams, CanopyTryCastParams, ParentCallbacks,
    copy_add_ref_params, copy_get_new_zone_id_params, copy_object_released_params, copy_post_params,
    copy_release_params, copy_send_params, copy_transport_down_params, copy_try_cast_params,
};
use canopy_rpc::{
    AddRefParams, GetNewZoneIdParams, IMarshaller, NewZoneIdResult, ObjectReleasedParams, PostParams, ReleaseParams,
    SendParams, SendResult, StandardResult, TransportDownParams, TryCastParams,
};
    ...
    M: IMarshaller,
{
    ...
    {
        self.marshaller.object_released(copy_object_released_params(raw));
    }

    pub fn transport_down(&self, raw: &CanopyTransportDownParams)
    {
        self.marshaller.transport_down(copy_transport_down_params(raw));
    }

    pub fn get_new_zone_id(&self, raw: &CanopyGetNewZoneIdParams) -> NewZoneIdResult
    {
        self.marshaller.get_new_zone_id(copy_get_new_zone_id_params(raw))
    }
}
...
mod tests
{
    use super::*;
    use crate::ffi::{
        CanopyBackChannelEntry, CanopyBackChannelSpan, CanopyConstByteBuffer, CanopyDllInitParams, borrow_remote_object,
        borrow_zone,
    };
    ...
    use canopy_rpc::{
        ...
        PostParams, ReleaseParams, SendParams, TransportDownParams, TryCastParams, Zone, ZoneAddress, ZoneAddressArgs,
    };
    use std::sync::Mutex;
    ...
    impl IMarshaller for RecordingMarshaller
    {
        ...
        {
            *self.last_send.lock().expect("last_send mutex should not be poisoned") = Some(params);
    ...
    {
        let zone = Zone::new(sample_zone_address());
        let remote = zone.with_object(Object::new(23)).expect("with_object should succeed");
        let marshaller = RecordingMarshaller::default();

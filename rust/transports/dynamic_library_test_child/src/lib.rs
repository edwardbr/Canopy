...
use canopy_rpc::{
    AddRefParams, AddressType, DefaultValues, GetNewZoneIdParams, NewZoneIdResult, Object, ObjectReleasedParams,
    PostParams, ReleaseParams, SendParams, SendResult, StandardResult, TransportDownParams, TryCastParams, Zone,
...
impl TestChildMarshaller
{
    ...
    {
        let stats = self.stats.lock().expect("stats mutex should not be poisoned");
        format!(
    ...
    fn call_parent_object_released(&self) -> SendResult
    {
        ...
        SendResult::new(error_codes::OK(), b"object-released-ok".to_vec(), Vec::new())
    }

...
namespace streaming
{
    // Linux acceptor that submits IORING_OP_ACCEPT directly to liburing and
    // wraps accepted sockets in iouring_stream instances.
    ...
    public:
        ...
        ~io_uring_stream_acceptor() override
        {
            stop();
            teardown_ring();
        }
        ...
        CORO_TASK(std::optional<std::shared_ptr<stream>>) accept() override
        {
            while (!stop_.load(std::memory_order_acquire))
            {
                ...
                while (!stop_.load(std::memory_order_acquire))
                {
                    ...
                    {
                        RPC_ERROR("io_uring_stream_acceptor: accept error {}", native_error);
                    }
                    CO_RETURN std::nullopt;
                }
            }

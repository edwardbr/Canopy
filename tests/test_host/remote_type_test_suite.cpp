...
template<class T> CORO_TASK(bool) coro_test_post_attribute(T& lib)
{
    ...
    // CO_AWAIT lib.get_scheduler()->yield_for(std::chrono::milliseconds(100));
    ...
    // CO_AWAIT lib.get_scheduler()->yield_for(std::chrono::milliseconds(200));
#else
...
TYPED_TEST(remote_type_test, test_post_attribute)
{

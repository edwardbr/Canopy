...
namespace rpc::ipc_transport
{
    ...
    queue_pair_bootstrap::queue_pair_bootstrap(std::string mapped_file, rpc::zone child_zone)
    ...
    child_host_bootstrap::child_host_bootstrap(std::string dll_path, std::string mapped_file, rpc::zone dll_zone)
        : queue_pair_bootstrap(std::move(mapped_file), dll_zone)
    ...
    std::shared_ptr<child_host_bootstrap> child_host_bootstrap::from_command_line(int argc, char** argv)
    {
        ...
        return std::make_shared<child_host_bootstrap>(args::get(dll_path_arg), args::get(mapped_file_arg), dll_zone);
    }
    ...
    child_process_bootstrap::child_process_bootstrap(std::string mapped_file, rpc::zone child_zone)
        : queue_pair_bootstrap(std::move(mapped_file), child_zone)
    {
    }
    ...
    std::shared_ptr<child_process_bootstrap> child_process_bootstrap::from_command_line(int argc, char** argv)
    {
        ...
        return std::make_shared<child_process_bootstrap>(args::get(mapped_file_arg), child_zone);
    }

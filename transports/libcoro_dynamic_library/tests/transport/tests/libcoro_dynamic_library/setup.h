...
public:
    ...
    CORO_TASK(bool) CoroSetUp()
    {
        ...
        this->root_service_ = std::make_shared<rpc::root_service>(
            "host", rpc::DEFAULT_PREFIX, this->io_scheduler_);
        ...
        auto connect_result
            = CO_AWAIT this->root_service_->template connect_to_zone<yyy::i_host, yyy::i_example>(
    ...
    virtual void set_up()
    {
        ...
        RPC_ASSERT(
            this->io_scheduler_->spawn_detached(this->check_for_error(CoroSetUp())));
    ...
    CORO_TASK(rpc::shared_ptr<yyy::i_example>) create_new_zone()
    {
        ...
        rpc::shared_ptr<yyy::i_example> example_relay_ptr
            = std::move(connect_result.output_interface);

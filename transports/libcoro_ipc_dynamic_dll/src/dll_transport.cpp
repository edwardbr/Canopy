...
namespace rpc::libcoro_ipc_dynamic_dll
{
    namespace
    {
        ...
        CORO_TASK(void) run_runtime(std::string name,
            rpc::zone dll_zone,
...
extern "C" CANOPY_LIBCORO_IPC_DLL_EXPORT void canopy_libcoro_ipc_dll_start(
    rpc::libcoro_ipc_dynamic_dll::dll_start_params* params,
    rpc::libcoro_ipc_dynamic_dll::dll_start_result* result)
{
    using namespace rpc::libcoro_ipc_dynamic_dll;

    auto* ctx = new runtime_context{};
    ctx->scheduler = std::shared_ptr<coro::scheduler>(coro::scheduler::make_unique(coro::scheduler::options{
    ...
    ctx->worker = std::thread([name = std::string(params->name ? params->name : "libcoro_ipc_dynamic_dll"),

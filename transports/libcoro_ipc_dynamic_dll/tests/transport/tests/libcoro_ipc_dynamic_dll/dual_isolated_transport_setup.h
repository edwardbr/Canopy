...
class libcoro_ipc_dll_dual_isolated_transport_setup
    : public libcoro_ipc_dll_setup_base<UseHostInChild, RunStandardTests, CreateNewZoneThenCreateSubordinatedZone>
{
    using base_type
        = libcoro_ipc_dll_setup_base<UseHostInChild, RunStandardTests, CreateNewZoneThenCreateSubordinatedZone>;

    typename base_type::isolated_child_process first_child_;
    typename base_type::isolated_child_process second_child_;
    std::shared_ptr<rpc::stream_transport::transport> second_client_transport_;
...
public:
    ...
    void set_up()
    {
        ...
        this->create_mapped_file(first_child_, "/tmp/canopy_ipc_dll_a_XXXXXX");
        this->create_mapped_file(second_child_, "/tmp/canopy_ipc_dll_b_XXXXXX");
        this->spawn_loader(first_child_);
        this->spawn_loader(second_child_);

        RPC_ASSERT(this->io_scheduler_->spawn_detached(this->check_for_error(
            this->connect_child("ipc child a", first_child_.queues, this->client_transport_, this->i_example_ptr_))));
        RPC_ASSERT(this->io_scheduler_->spawn_detached(this->check_for_error(
            this->connect_child("ipc child b", second_child_.queues, second_client_transport_, second_example_ptr_))));
